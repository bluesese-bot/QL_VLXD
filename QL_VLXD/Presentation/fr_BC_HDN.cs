...
{
    public partial class fr_BC_HDN : Form
    {
        public fr_BC_HDN()
        {
            InitializeComponent();
        ...
        private void fr_BC_HDN_Load(object sender, EventArgs e)
        {
            hienthi();
            khoitaoluoi();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            exRange.Range["A1:B1"].Value = "Cửa Hàng Vật Liệu Xây Dựng";
            ...
            exRange.Range["F11:F11"].Value = "Ghi Chú";
        ...
        private void txtthongtin_TextChanged(object sender, EventArgs e)
        {
            if (op1.Checked)
            {
                string sql = @"SELECT dbo.tblHoaDonNhap.MaHD,dbo.tblHoaDonNhap.MaNCC,dbo.tblHoaDonNhap.NgayNhap,dbo.tblHoaDonNhap.TongTien,dbo.tblHoaDonNhap.GhiChu FROM dbo.tblHoaDonNhap INNER JOIN dbo.tblChiTietHDN ON tblChiTietHDN.MaHD = tblHoaDonNhap.MaHD WHERE dbo.tblChiTietHDN.MaMatH=N" + txtthongtin.Text + "'";
                msds.DataSource = cn.taobang(sql);
            ...
            if (op2.Checked)
            {
                string sql = @"SELECT dbo.tblHoaDonNhap.MaHD,dbo.tblHoaDonNhap.MaNCC,dbo.tblHoaDonNhap.NgayNhap,dbo.tblHoaDonNhap.TongTien,dbo.tblHoaDonNhap.GhiChu FROM dbo.tblHoaDonNhap INNER JOIN dbo.tblChiTietHDN ON tblChiTietHDN.MaHD = tblHoaDonNhap.MaHD WHERE dbo.tblHoaDonNhap.NgayNhap=N'" + txtthongtin.Text + "'";
                msds.DataSource = cn.taobang(sql);
            ...
            if (op3.Checked)
            {
                string sql = @"SELECT dbo.tblHoaDonNhap.MaHD,dbo.tblHoaDonNhap.MaNCC,dbo.tblHoaDonNhap.NgayNhap,dbo.tblHoaDonNhap.TongTien,dbo.tblHoaDonNhap.GhiChu FROM dbo.tblHoaDonNhap INNER JOIN dbo.tblChiTietHDN ON tblChiTietHDN.MaHD = tblHoaDonNhap.MaHD WHERE dbo.tblHoaDonNhap.MaNCC=N'" + txtthongtin.Text + "'";
                msds.DataSource = cn.taobang(sql);
        ...
        private void msds_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //Lấy thông tin các mặt hàng
            dong = e.RowIndex;
            sql = @"SELECT * FROM dbo.tblHoaDonNhap WHERE MaHD = '" + msds.Rows[dong].Cells[0].Value.ToString() + "'";
            tblThongtinHang = cn.taobang(sql);
            ...
            //Tạo dòng tiêu đề bảng
            sql = @"SELECT * FROM dbo.tblChiTietHDN WHERE MaHD = '" + msds.Rows[dong].Cells[0].Value.ToString() + "'";
