...
namespace QL_VLXD.Presentation
{
    public partial class fr_TaiKhoan : Form
    {
        ...
        {
            cn.LoadLenCombobox(cbTenNhanVien, "SELECT MaNhanVien FROM dbo.tblNhanVien", 0);
            locktext();
            hienthi();
            khoitaoluoi();
        }
        ...
        {
            label6.Text = cn.LoadLable("SELECT TenNhanVien FROM dbo.tblNhanVien WHERE MaNhanVien ="+cbTenNhanVien.Text+"");
        }
    }
}
