...
namespace QL_VLXD.Presentation
{
    public partial class fr_BC_HDB : Form
    {
        ...
        private void txtthongtin_TextChanged(object sender, EventArgs e)
        {
            if (op1.Checked)
            {
                string sql = @"SELECT dbo.tblHoaDonXuat.MaHD,dbo.tblHoaDonXuat.MaNhanVien,dbo.tblHoaDonXuat.NgayXuat,dbo.tblHoaDonXuat.TongTien,dbo.tblHoaDonXuat.GhiChu FROM dbo.tblHoaDonXuat INNER JOIN dbo.tblChiTietHDX ON tblChiTietHDX.MaHD = tblHoaDonXuat.MaHD WHERE dbo.tblChiTietHDX.MaMH=N'" + txtthongtin.Text + "'";
                msds.DataSource = cn.taobang(sql);
            ...
            if (op2.Checked)
            {
                string sql = @"SELECT dbo.tblHoaDonXuat.MaHD,dbo.tblHoaDonXuat.MaNhanVien,dbo.tblHoaDonXuat.NgayXuat,dbo.tblHoaDonXuat.TongTien,dbo.tblHoaDonXuat.GhiChu FROM dbo.tblHoaDonXuat INNER JOIN dbo.tblChiTietHDX ON tblChiTietHDX.MaHD = tblHoaDonXuat.MaHD WHERE dbo.tblHoaDonXuat.NgayXuat=N'" + txtthongtin.Text + "'";
                msds.DataSource = cn.taobang(sql);
            ...
            if (op3.Checked)
            {
                string sql = @"SELECT dbo.tblHoaDonXuat.MaHD,dbo.tblHoaDonXuat.MaNhanVien,dbo.tblHoaDonXuat.NgayXuat,dbo.tblHoaDonXuat.TongTien,dbo.tblHoaDonXuat.GhiChu FROM dbo.tblHoaDonXuat INNER JOIN dbo.tblChiTietHDX ON tblChiTietHDX.MaHD = tblHoaDonXuat.MaHD WHERE dbo.tblHoaDonXuat.MaNhanVien=N'" + txtthongtin.Text + "'";
                msds.DataSource = cn.taobang(sql);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            exRange.Range["D11:D11"].Value = "Ngày Xuất";
        ...
        private void msds_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //Lấy thông tin các mặt hàng
            dong = e.RowIndex;
            sql = @"SELECT * FROM dbo.tblHoaDonXuat WHERE MaHD = '" + msds.Rows[dong].Cells[0].Value.ToString() + "'";
            tblThongtinHang = cn.taobang(sql);
            ...
            //Tạo dòng tiêu đề bảng
            sql = @"SELECT * FROM dbo.tblChiTietHDX WHERE MaHD = '" + msds.Rows[dong].Cells[0].Value.ToString() + "'";
